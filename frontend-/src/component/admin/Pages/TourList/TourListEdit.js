...
function TourListEdit() {
  ...
  useEffect(() => {
    let fatchData = async () => {
      await axios.get(`/tour/${id}`).then((x) => {
        console.log(id)
        setUserData(x.data)
                ...
                <Formik
                  ...
                  onSubmit={async (values) => {
                   ...
                   // formdata.append('tourpageRefid', values.tourpageRefid)
                    ...
                    await axios.put(`/tour/editApi/${id}`,formdata).then((x) => navigate('/admin/dashboard/tour-list'))
                  ...
                  {({ values, setFieldValue }) => (
                    ...
                    <Form>
                      ...
                      <div className="mb-3 mt-3 row g-9 align-items-center">
                        ...
                        <div className="col">
                          <CKEditor name='tourHlView' editor={ClassicEditor} data={userData.tourHlView}
                      ...
                      <div className="mb-3 mt-3 row g-9 align-items-center">
                        ...
                        <div className="col">
                          <CKEditor name='tourHlPoint' editor={ClassicEditor} data={userData.tourHlPoint}
                            onChange={(event, editor) => {
                              const data = editor.getData();
                      ...
                      <div className="mb-3 mt-3 row g-9 align-items-center">
                        ...
                        <div className="col">
                          <CKEditor name='tourDtls' editor={ClassicEditor} data={userData.tourDtls}
                            onChange={(event, editor) => {
                              const data = editor.getData();
                      ...
                      <div className="mb-3 mt-3 row g-9 align-items-center">
                        <div className="col col-lg-2 mt-0"><label className="form-label">Day Tour</label></div>
                        <FieldArray
                          name="dayTours"
                          render={arrayHelpers => (
                              ...
                              {values.dayTours && values.dayTours.length > 0 ? (
                                values.dayTours.map((dayTour, index) => (
                                  <div className='border-bottom border border-1  p-2 border-bottom-0 bg-light p-4' key={index}>
                                    <div className='d-flex justify-content-center pt-0 pb-1'><div className="bg-dark ps-3 pe-3 pt-2 pb-2  text-white">Day {index + 1}</div></div>
                                    <Field name={`dayTours.${index}.dayCont`} placeholder='Tour Day' className="form-control pb-2" />
                                    <CKEditor name={`dayTours.${index}.dayDtls`} editor={ClassicEditor}
                                      data={daytrip[index].dayDtls}
